! All copyright for this module goes to https://github.com/tfgg/zeromq_fortran
module zmq
...
!GCC$ ATTRIBUTES DLLEXPORT :: sc_getInitData
   ...
   do j = 1, nTurbines
      ParamTurbine(j) = real(j*0.005, C_FLOAT)
   end do

   do j = 1, nTurbines
      from_SC(j) = real( ParamTurbine(j), C_FLOAT)
   end do
...
!GCC$ ATTRIBUTES DLLEXPORT :: sc_calcOutputs
   ...
   ! zeroMQ stuff
   IF (mod(t,tCall)<1e-8) THEN ! Only call every tCall seconds
		context = zmq_ctx_new()
		requester = zmq_socket(context, ZMQ_REQ)

		write(*,*) "Connecting to server"

		rc = zmq_connect(requester, "tcp://localhost:5553")
		...
		write(*,*) "Received ", recv_buffer(1:10), request_nbr
		...
		! ---
   ELSE
		!write(*,*) "Skipping SSC for time:",t
   END IF
	
   do j = 1, nTurbines  
      do i = 1, NumSC2Ctrl
         from_SC((j-1)*NumSC2Ctrl+i) = paramTurbine(j) * t ! The pitch is the pitch rate times the time
      end do
   end do
